...
{
	public class ColumnrangeSeriesPointEvents : BaseObject
	{
		public string Click
		{
			get;
		...
		public ColumnrangeSeriesPointEvents()
		{
			string str = "";
			string str1 = str;
		...
		internal override Hashtable ToHashtable()
		{
			...
			if (this.Click != this.Click_DefaultValue)
			{
				...
				Highcharts.AddFunction("ColumnrangeSeriesPointEventsClick.click", this.Click);
			}
			if (this.MouseOut != this.MouseOut_DefaultValue)
			{
				hashtables.Add("mouseOut", this.MouseOut);
				Highcharts.AddFunction("ColumnrangeSeriesPointEventsMouseOut.mouseOut", this.MouseOut);
			}
			if (this.MouseOver != this.MouseOver_DefaultValue)
			{
				hashtables.Add("mouseOver", this.MouseOver);
				Highcharts.AddFunction("ColumnrangeSeriesPointEventsMouseOver.mouseOver", this.MouseOver);
			}
			if (this.Remove != this.Remove_DefaultValue)
			{
				hashtables.Add("remove", this.Remove);
				Highcharts.AddFunction("ColumnrangeSeriesPointEventsRemove.remove", this.Remove);
			}
			if (this.Select != this.Select_DefaultValue)
			{
				hashtables.Add("select", this.Select);
				Highcharts.AddFunction("ColumnrangeSeriesPointEventsSelect.select", this.Select);
			}
			if (this.Unselect != this.Unselect_DefaultValue)
			{
				hashtables.Add("unselect", this.Unselect);
				Highcharts.AddFunction("ColumnrangeSeriesPointEventsUnselect.unselect", this.Unselect);
			}
			if (this.Update != this.Update_DefaultValue)
			{
				hashtables.Add("update", this.Update);
				Highcharts.AddFunction("ColumnrangeSeriesPointEventsUpdate.update", this.Update);
			}
			return hashtables;
		}
