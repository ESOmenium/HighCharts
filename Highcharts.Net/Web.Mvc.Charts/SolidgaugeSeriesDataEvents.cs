...
{
	public class SolidgaugeSeriesDataEvents : BaseObject
	{
		public string Click
		{
			get;
		...
		internal override Hashtable ToHashtable()
		{
			...
			if (this.Click != this.Click_DefaultValue)
			{
				...
				Highcharts.AddFunction("SolidgaugeSeriesDataEventsClick.click", this.Click);
			}
			if (this.MouseOut != this.MouseOut_DefaultValue)
			{
				hashtables.Add("mouseOut", this.MouseOut);
				Highcharts.AddFunction("SolidgaugeSeriesDataEventsMouseOut.mouseOut", this.MouseOut);
			}
			if (this.MouseOver != this.MouseOver_DefaultValue)
			{
				hashtables.Add("mouseOver", this.MouseOver);
				Highcharts.AddFunction("SolidgaugeSeriesDataEventsMouseOver.mouseOver", this.MouseOver);
			}
			if (this.Remove != this.Remove_DefaultValue)
			{
				hashtables.Add("remove", this.Remove);
				Highcharts.AddFunction("SolidgaugeSeriesDataEventsRemove.remove", this.Remove);
			}
			if (this.Select != this.Select_DefaultValue)
			{
				hashtables.Add("select", this.Select);
				Highcharts.AddFunction("SolidgaugeSeriesDataEventsSelect.select", this.Select);
			}
			if (this.Unselect != this.Unselect_DefaultValue)
			{
				hashtables.Add("unselect", this.Unselect);
				Highcharts.AddFunction("SolidgaugeSeriesDataEventsUnselect.unselect", this.Unselect);
			}
			if (this.Update != this.Update_DefaultValue)
			{
				hashtables.Add("update", this.Update);
				Highcharts.AddFunction("SolidgaugeSeriesDataEventsUpdate.update", this.Update);
			}
			return hashtables;
		}
