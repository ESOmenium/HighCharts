...
{
	public class PolygonSeriesPointEvents : BaseObject
	{
		public string Click
		{
			get;
		...
		public PolygonSeriesPointEvents()
		{
			string str = "";
			string str1 = str;
		...
		internal override Hashtable ToHashtable()
		{
			...
			if (this.Click != this.Click_DefaultValue)
			{
				...
				Highcharts.AddFunction("PolygonSeriesPointEventsClick.click", this.Click);
			}
			if (this.MouseOut != this.MouseOut_DefaultValue)
			{
				hashtables.Add("mouseOut", this.MouseOut);
				Highcharts.AddFunction("PolygonSeriesPointEventsMouseOut.mouseOut", this.MouseOut);
			}
			if (this.MouseOver != this.MouseOver_DefaultValue)
			{
				hashtables.Add("mouseOver", this.MouseOver);
				Highcharts.AddFunction("PolygonSeriesPointEventsMouseOver.mouseOver", this.MouseOver);
			}
			if (this.Remove != this.Remove_DefaultValue)
			{
				hashtables.Add("remove", this.Remove);
				Highcharts.AddFunction("PolygonSeriesPointEventsRemove.remove", this.Remove);
			}
			if (this.Select != this.Select_DefaultValue)
			{
				hashtables.Add("select", this.Select);
				Highcharts.AddFunction("PolygonSeriesPointEventsSelect.select", this.Select);
			}
			if (this.Unselect != this.Unselect_DefaultValue)
			{
				hashtables.Add("unselect", this.Unselect);
				Highcharts.AddFunction("PolygonSeriesPointEventsUnselect.unselect", this.Unselect);
			}
			if (this.Update != this.Update_DefaultValue)
			{
				hashtables.Add("update", this.Update);
				Highcharts.AddFunction("PolygonSeriesPointEventsUpdate.update", this.Update);
			}
			return hashtables;
		}
