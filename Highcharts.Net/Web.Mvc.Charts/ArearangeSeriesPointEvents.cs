...
{
	public class ArearangeSeriesPointEvents : BaseObject
	{
		...
		internal override Hashtable ToHashtable()
		{
			...
			if (Click != Click_DefaultValue)
			{
				...
				Highcharts.AddFunction("ArearangeSeriesPointEventsClick.click", Click);
			}
			if (MouseOut != MouseOut_DefaultValue)
			{
				hashtables.Add("mouseOut", MouseOut);
				Highcharts.AddFunction("ArearangeSeriesPointEventsMouseOut.mouseOut", MouseOut);
			}
			if (MouseOver != MouseOver_DefaultValue)
			{
				hashtables.Add("mouseOver", MouseOver);
				Highcharts.AddFunction("ArearangeSeriesPointEventsMouseOver.mouseOver", MouseOver);
			}
			if (Remove != Remove_DefaultValue)
			{
				hashtables.Add("remove", Remove);
				Highcharts.AddFunction("ArearangeSeriesPointEventsRemove.remove", Remove);
			}
			if (Select != Select_DefaultValue)
			{
				hashtables.Add("select", Select);
				Highcharts.AddFunction("ArearangeSeriesPointEventsSelect.select", Select);
			}
			if (Unselect != Unselect_DefaultValue)
			{
				hashtables.Add("unselect", Unselect);
				Highcharts.AddFunction("ArearangeSeriesPointEventsUnselect.unselect", Unselect);
			}
			if (Update != Update_DefaultValue)
			{
				hashtables.Add("update", Update);
				Highcharts.AddFunction("ArearangeSeriesPointEventsUpdate.update", Update);
			}
			return hashtables;
		}
