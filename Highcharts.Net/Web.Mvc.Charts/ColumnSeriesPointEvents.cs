...
{
	public class ColumnSeriesPointEvents : BaseObject
	{
		public string Click
		{
			get;
		...
		public ColumnSeriesPointEvents()
		{
			string str = "";
			string str1 = str;
		...
		internal override Hashtable ToHashtable()
		{
			...
			if (this.Click != this.Click_DefaultValue)
			{
				...
				Highcharts.AddFunction("ColumnSeriesPointEventsClick.click", this.Click);
			}
			if (this.MouseOut != this.MouseOut_DefaultValue)
			{
				hashtables.Add("mouseOut", this.MouseOut);
				Highcharts.AddFunction("ColumnSeriesPointEventsMouseOut.mouseOut", this.MouseOut);
			}
			if (this.MouseOver != this.MouseOver_DefaultValue)
			{
				hashtables.Add("mouseOver", this.MouseOver);
				Highcharts.AddFunction("ColumnSeriesPointEventsMouseOver.mouseOver", this.MouseOver);
			}
			if (this.Remove != this.Remove_DefaultValue)
			{
				hashtables.Add("remove", this.Remove);
				Highcharts.AddFunction("ColumnSeriesPointEventsRemove.remove", this.Remove);
			}
			if (this.Select != this.Select_DefaultValue)
			{
				hashtables.Add("select", this.Select);
				Highcharts.AddFunction("ColumnSeriesPointEventsSelect.select", this.Select);
			}
			if (this.Unselect != this.Unselect_DefaultValue)
			{
				hashtables.Add("unselect", this.Unselect);
				Highcharts.AddFunction("ColumnSeriesPointEventsUnselect.unselect", this.Unselect);
			}
			if (this.Update != this.Update_DefaultValue)
			{
				hashtables.Add("update", this.Update);
				Highcharts.AddFunction("ColumnSeriesPointEventsUpdate.update", this.Update);
			}
			return hashtables;
		}
